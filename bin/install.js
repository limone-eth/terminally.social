#!/usr/bin/env node
// Wires claude-presence into your real Claude Code setup (~/.claude/settings.json).
//
// - Backs up settings.json first (settings.json.claude-presence-backup-<ts>)
// - Moves your current statusLine command into claude-presence's config as
...
const backupPath = `${SETTINGS_PATH}.claude-presence-backup-${Date.now()}`
...
if (uninstall) {
  ...
  console.log('claude-presence hooks and statusline removed')
  process.exit(0)
}
